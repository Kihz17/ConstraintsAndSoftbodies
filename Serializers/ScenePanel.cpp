...
void ScenePanel::OnUpdate(float deltaTime)
{
	...
	if (this->meshData)
	{
		...
		{
			std::string tPath("assets\\textures\\" + this->textureName);
			...
			{
				Ref<DiffuseTexture> texture = TextureManager::LoadDiffuseTexture(tPath, filterType, wrapType, this->genMipMaps);
				this->meshData->AddTexture(texture);
			}
			else if (textureTypeString == "Heightmap")
			{
				Ref<HeightMapTexture> texture = TextureManager::LoadHeightmapTexture(tPath, filterType, wrapType, glm::vec3(0.0f, 0.0f, 0.0f), 1000.0f);
				this->meshData->AddTexture(texture);
			}
			else if (textureTypeString == "Discard")
			{
				Ref<DiscardTexture> texture = TextureManager::LoadDiscardTexture(tPath, filterType, wrapType, this->genMipMaps);
				this->meshData->AddTexture(texture);
			}
			else if (textureTypeString == "Alpha")
			{
				Ref<AlphaTexture> texture = TextureManager::LoadAlphaTexture(tPath, filterType, wrapType, this->genMipMaps);
				this->meshData->AddTexture(texture);
			}
		}
